...
public class BatchPaymentFuture {
    ...
    public static void main(String[] args) throws ExecutionException, InterruptedException {
        ...
        System.out.print("并发查询订单: ");
        for (Future<Order> order : orderFutures) {
            Order o = order.get();
            System.out.print(o.id + " (" + o.amount + ")  ");
        }
        System.out.println();
        System.out.println("订单并发查询完成    -------");
        ...
        System.out.print("并发支付订单: ");
        for (Future<Boolean> payment : paymentFutures) {
            boolean p = payment.get();
            System.out.print(p + "  ");
        }
        System.out.println();
        System.out.println("订单支付完成       -------");

        System.out.println("并发发送邮件: ");
        for (int i = 0; i < paymentFutures.size(); i++) {
            Order order = orderFutures.get(i).get();
            boolean paymentResult = paymentFutures.get(i).get();
            executorService.submit(() -> sendEmail(order, paymentResult));
        }
