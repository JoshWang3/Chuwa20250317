...
public class InterruptTest {
    ...
    private void execute(Thread t) throws InterruptedException {
        t.start();
        Thread.sleep(2); // 暂停2毫秒. Question, who sleep?  Answer: 当前线程
        t.interrupt(); // 中断t线程. Question, who is interrupted? 一定会被成功中断么？  Answer: t线程，如果t线程中根据isInterrupt()退出循环，t线程就会结束，t.interrupt()只是设置线程的状态，并不主动中断进程
        t.join(); // 等待t线程结束. Question, who is running, and who stopped?  Answer: 当前线程还在运行，t线程结束
...
class MyThreadWithIsInterrupted extends Thread {
    ...
    public void run() {
        ...
        // Answer: 可以被中断，因为线程的 interrupted 状态变了，主动跳出循环，结束线程
...
class WhileTrue extends Thread {
    ...
    public void run() {
        int n = 0;
        // Question, can be interrupted?
        // Answer: 不能，因为 t.interrupt 不会主动中断线程
        while (true) {
            n++;
            System.out.println(n + " hello!");
        }
...
class SleepForever extends Thread {
    ...
    public void run() {
        int n = 0;
        // Question, can be interrupted?
        // Answer: 会被中断，因为如果线程的状态是 WAITING/TIMED_WAITING，会抛出中断异常
        try {
