...
Runnable: interfaces, no return, no throw exceptions
Callable: interfaces, return, throw exceptions
...
Runable: more flexible, Java not allow extend mult class, but allows implements mult interface.
...
synchronized: allow only one thread access or change the data at a time.
wait(), notify(), and use volatile to ensures that all threads see the latest value when they check it. 
...
1. 2. 3. Homework 1: Write a simple program that uses CompletableFuture to asynchronously get the sum
