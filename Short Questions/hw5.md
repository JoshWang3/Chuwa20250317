...
```
API-GET-2
- API: GET GET https://httpstat.us/500?sleep=1000
- Description: Simulates a delayed server error response for testing purposes.
...
```
API-GET-3
- API: GET https://www.w3schools.com/xml/missing.xml
- Description: Attempts to retrieve a non-existent XML file to simulate a 404 Not Found scenario.
...
<body>
            ...
            <svg width="40" height="38" viewBox="0 0 40 38" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g clip-path="url(#clip0)">
                    ...
                    <path
                        ...
                        fill="#04AA6D" />
                    <path d="M32.6553 37.4212V29.1018H33.9966V37.4212H32.6553Z" fill="#04AA6D" />
                    <path
    ...
    <div class="w3-row">
        <div class="w3-container w3-half" style="padding: 100px 0px 100px 50px;">
            <h2 style="font-size: 30px;">Sorry!</h2>
            <h1 style="font-weight:bold; font-size: 36px;">We can't seem to find the resource you're looking for.</h1>
            <p style="font-size: 16px; opacity: 0.7">Please check that the Web site address is spelled correctly.<br>Or go to our
                <a href="/default.asp">home page</a>, and use the menus to navigate to a specific section.</p>
        </div>
        <div class="w3-container w3-half" style="padding: 100px;">
            <div
                style="position:relative;display:block;width:312px;height:312px;background-color:#d6eee2;-webkit-border-radius:50%;-moz-border-radius:50%;border-radius:50%;">
                <svg class="-smile-icon"
                    style="width:148px;height:217px;position:absolute;top:50%;left:50%;margin-top:-108.5px;margin-left:-74px"
                    width="142" height="210" viewBox="0 0 142 210" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
                    ...
                    <path
                        ...
                        fill="#04AA6D" />
                    <path d="M4.22766 170.309H18.3328V192.514H127.895V170.309H142V209.227H4.22766V170.309Z"
                        fill="#04AA6D" />
                </svg>
            </div>
        </div>
...
```
API-GET-4: HTML Response
- API: GET https://example.com
- Description: Retrieves the HTML content of the Example domain.
...
```
API-GET-5: Image Response
- API: GET https://cataas.com/cat
- Description: Retrieves a randow cat image.
- Response Status: 200 OK
- Response Body:
![response body kitty](./hw5-image-resonse-body.png)

API-POST-1: Create a New Post
- API: POST https://jsonplaceholder.typicode.com/posts
- `404 Not Found` if wrong path `/postz` with response body `{}`
- Request Body:
```json
{
  "title": "dummy",
...
```json
{
    ...
    "job": "sde",
    "id": "2",
    "createdAt": "2025-04-04T22:12:51.693Z"
}
```
API-POST-3: Create User
...
```
API-POST-4: Login User
- API: POST https://reqres.in/api/login
- Request Body:
  ```json
  {
  "email": "eve.holt@reqres.in",
  "password": "cdfdfdfdfiti"
  }
  ```
- Response Status: 200 OK
- Response Body:
...
- Response Body:
  ...
  ```
API-PUT-2
- API: PUT https://httpbin.org/status/401
- Request Body:
  ```json
  {
  "username": "elena.xu@icloud.com",
...
API-PUT-3
- API: PUT https://jsonplaceholder.typicode.com/posts/999999
- Request Body: 
  ```json
  {
  "id": 999999,
  "title": "Updated Post That Doesn’t Exist",
  "body": "This is a test update to a post that likely isn't real.",
  "userId": 1
  }
  ```
- Response Status: `500 Internal Server Error`
...
API-DELETE-2 
- API: https://jsonplaceholder.typicode.com/postz/1
- Response Body: `{}`
- Response Status: `404 Not Found`
...
```http
GET /customers/{customerId}/default-payment-method
...
- Twitter API (v2)
  ...
  - GET /tweets/:id
  - /2/users/:id/tweets
- Summary Naming Conventioins
  - RESTful (uses HTTP verbs like GET, POST, DELETE)
  - Uses plural nouns to represent resources (/tweets, /users)
  - Clear resource hierarchy (e.g., /users/:id/followers)
  - Avoids verbs in path — actions are represented by HTTP method
  - Includes version in the path (/2/) 
...
- Summary
  - Use path variables	-> To specify unique resources /users/{userId}
  - Use query params	-> For filters, sorting, pagination
  - Use sub-resources	-> When a resource is tightly coupled to a parent (/posts/{id}/comments)
  - Use plurals	-> For collections: /comments, /posts
  - Use singular	-> Only when the resource is logically a singleton: /profile, /settings